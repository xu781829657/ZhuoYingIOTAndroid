package com.ouzhongiot.ozapp.esptouch.task;
...
import com.ouzhongiot.ozapp.esptouch.EsptouchResult;
import com.ouzhongiot.ozapp.esptouch.IEsptouchListener;
import com.ouzhongiot.ozapp.esptouch.IEsptouchResult;
import com.ouzhongiot.ozapp.esptouch.protocol.EsptouchGenerator;
import com.ouzhongiot.ozapp.esptouch.udp.UDPSocketClient;
import com.ouzhongiot.ozapp.esptouch.udp.UDPSocketServer;
import com.ouzhongiot.ozapp.esptouch.util.ByteUtil;
import com.ouzhongiot.ozapp.esptouch.util.EspNetUtil;
	...
	private void __putEsptouchResult(boolean isSuc, String bssid,
		...
		synchronized (mEsptouchResultList) {
			...
			if (__IEsptouchTask.DEBUG) {
				Log.wtf(TAG, "__putEsptouchResult(): count = " + count);
			}
		...
		new Thread() {
			public void run() {
				...
				while (mEsptouchResultList.size() < mParameter
					...
					Log.wtf("这个是收到的UDP信息长度",i+"");
					String udpdata = "";
                     for (int j=0;j<i;j++){


						 String bytedata = Integer.toHexString(0x000000FF & receiveBytes[j]);

							 Log.wtf("16进制为",bytedata);

						 udpdata = udpdata+bytedata;

					 }
					Log.wtf("这个是收到的UDP信息",udpdata);
					}
					if (receiveBytes != null) {
					...
					if (receiveOneByte == expectOneByte) {
						...
						if (__IEsptouchTask.DEBUG) {
							Log.i(TAG, "receive rubbish MessageNotificationActivity, just ignore");
						}
					}
				}
